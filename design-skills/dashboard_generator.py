...
# Import Chart.js bridge for interactive charts (MT-52/E2, S183)
try:
    from chartjs_bridge import (
        bar_chart_config, line_chart_config, donut_chart_config,
        render_chartjs_canvas, render_chartjs_script, render_chartjs_cdn_tag,
    )
    CHARTJS_AVAILABLE = True
except ImportError:
...
class DashboardRenderer:
    ...
    def render(self, data: DashboardData, theme: str = "light", refresh_seconds: int = 0, interactive: bool = False) -> str:
        ...
        interactive_html = self._render_interactive_charts(data) if interactive else ""
        chartjs_cdn = render_chartjs_cdn_tag() if CHARTJS_AVAILABLE and interactive_html else ""
...
{self._js()}
    ...
    def render_to_file(self, data: DashboardData, path: str, theme: str = "light", refresh_seconds: int = 0, interactive: bool = False):
        """Render and write to file."""
        content = self.render(data, theme=theme, refresh_seconds=refresh_seconds, interactive=interactive)
        tmp = path + ".tmp"
...
{chr(10).join(charts)}
    ...
    def _render_interactive_charts(self, data) -> str:
        """Render Chart.js interactive charts (MT-52/E2, S183).

        Generates hover-tooltips, responsive sizing, and animations.
        Appears below the static SVG charts section.
        """
        if not CHARTJS_AVAILABLE or not data.modules:
            return ""

        parts = []

        # Tests by module (interactive bar)
        module_data = sorted(
            [(m.name, m.tests) for m in data.modules],
            key=lambda x: x[1], reverse=True,
        )
        if module_data:
            labels = [m[0] for m in module_data]
            values = [m[1] for m in module_data]
            config = bar_chart_config(labels, values, title="Tests by Module (Interactive)", horizontal=True)
            parts.append(render_chartjs_canvas("chart_tests_module", width=500, height=max(250, len(module_data) * 35)))
            parts.append(render_chartjs_script("chart_tests_module", config))

        # Task status donut (interactive)
        if data.master_tasks:
            complete = sum(1 for t in data.master_tasks if "COMPLETE" in t.status.upper())
            in_progress = sum(1 for t in data.master_tasks
                              if "PROGRESS" in t.status.upper() or "ACTIVE" in t.status.upper())
            ...
            if other:
                ...
                values.append(other)
            if labels:
                config = donut_chart_config(labels, values, title="Master Task Status (Interactive)")
                parts.append(render_chartjs_canvas("chart_task_donut", width=350, height=300))
                parts.append(render_chartjs_script("chart_task_donut", config))

        # Test growth line chart (if session timeline data exists)
        if hasattr(data, "metrics") and data.metrics:
            test_total = next((m.value for m in data.metrics if "test" in m.label.lower()), None)
            session_num = next((m.value for m in data.metrics if "session" in m.label.lower()), None)
            if test_total and session_num:
                # Simple 5-point projection based on current total
                try:
                    total = int(str(test_total).replace(",", ""))
                    sess = int(str(session_num).replace(",", "").replace("S", ""))
                    if sess > 5:
                        labels = [f"S{sess - 4}", f"S{sess - 3}", f"S{sess - 2}", f"S{sess - 1}", f"S{sess}"]
                        step = max(1, total // sess)
                        values = [total - step * 4, total - step * 3, total - step * 2, total - step, total]
                        config = line_chart_config(labels, values, title="Test Growth Trend",
                                                   fill=True, smooth=True, series_name="Total Tests")
                        parts.append(render_chartjs_canvas("chart_test_growth", width=500, height=250))
                        parts.append(render_chartjs_script("chart_test_growth", config))
        ...
        return f"""<h2 class="section-header">Interactive Charts</h2>
<div class="charts">
{chr(10).join(parts)}
</div>"""
...
def cli_main(args: list = None):
    ...
    if cmd == "generate":
        ...
        renderer.render_to_file(data, output, theme=theme, refresh_seconds=refresh, interactive=interactive)

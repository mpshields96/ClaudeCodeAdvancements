...
| Design Skills | `design-skills/` | MT-17 Phase 5 + daily snapshots + trading_chart (MT-24) + 22 chart types + consistency audit + report_charts (wired into /cca-report, +4 statistical MT-32, +CalibrationPlot S190) + BubbleChart + TreemapChart + SankeyChart + ScatterPlot + BoxPlot + HistogramChart + ViolinPlot + CalibrationPlot + kalshi_data_collector + learning_data_collector + report_sidecar + report_differ (MT-33) + Dashboard v2 (dark mode, sortable, search, collapsible) + figure integration (slides/dashboard/website, MT-32 Phase 7) + chartjs_bridge (MT-52/E2, Chart.js interactive charts) | 1460 |
| Research | `research/` | Reddit scout, MT-8/MT-13 Phase 2 COMPLETE, Agent Orchestration Research (S190) | 86 |

**Total: ~10,252 tests (~266 suites). All must pass before any work.**
